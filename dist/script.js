...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var calcFunction = function calcFunction() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      resultBlock.textContent = 'Пожалуйста, выберите размеp и материал картины';
...
var drop = function drop() {
  /*
      drag *
      dragend *
      dragenter - переиаскиваемый объкет
  перетаскивается над dropArea
      dragexit *
      dragleave - за пределами dropArea
      dragover - объект зависает и двигается 
  над dropArea
      dragstart *
      drop - объект отправлен в dropArea
  .* срабатывают на эл-те, который мы перетаскиваем
   */
...
var filter = function filter() {
  ...
  var filtered = function filtered(selector) {
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  function showImg(block) {
    var img = block.querySelector('img'); // img.src = `${img.src.split('.')[0]}-1.${img.src.split('.')[1]}`;
  ...
  function hideImg(block) {
    var img = block.querySelector('img'); // img.src = `${img.src.split('-1')[0]}${img.src.split('-1')[1]}`;
